...
def extract():
    with open ('load.txt', 'rt') as in_file: # Open file txt for reading of text data.
        for line in in_file:
...
def train(datapath,layer):
    ...
    model.summary()
    model.fit(X, y_train, batch_size=10, epochs=train_cycle)  
    
    return model,c_c,activate,threshold

def trainexisting(datapath,layer,activate,train_cycle,row,col,bands,dtype,dtypevalue,threshold,c_c):

    epoch=int(train_cycle)
    values = []
    c_l={}
    path=datapath
    c=0
...
def saveweight(c_c,datapath,layer,model_new,activate,threshold):
    ...
    if MsgBox=="yes":
        ...
        for i in range(0,len(datapath)):
            file.write(str(i+1)+","+str(datapath[i])+"\n")
            i=i+1
        for i in range(0,len(layer)):
            file.write(layer[i]+"\n")        
...
def trainmodelexistin(dataaddedmore,epoch,thresh,headerfile,weightfile):
    row,col,band,dtype,dvalue=headerreader(headerfile)
    c_c,datapath,layer,activate,threshold=extractdata(weightfile)
    c_cnew=c_c+len(dataaddedmore)
    print("c_c_ is")
    print(c_cnew)
    datapathnew = []
    for i in range(0,len(datapath)):
        datapathnew.append(datapath[i])
    for i in range(0,len(dataaddedmore)):
        datapathnew.append(dataaddedmore[i])
    print(datapathnew)
    model_new,c_c_save,activate_save,threshold_save=trainexisting(datapathnew,layer,activate,epoch,row,col,band,dtype,dvalue,thresh,c_cnew)
    saveweight(c_c_save,datapathnew,layer,model_new,activate_save,threshold_save) 